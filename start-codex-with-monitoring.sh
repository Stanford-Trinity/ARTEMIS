...
AUTONOMOUS_DURATION="${AUTONOMOUS_DURATION:-1440}"  # 24 hours in minutes
...
if [[ ! -f "$AUTONOMOUS_CONFIG" ]]; then
    error "Autonomous config file not found: $AUTONOMOUS_CONFIG"
    exit 1
fi
...
python3 "$HEALTH_MONITOR_SCRIPT" --config "$HEALTH_CONFIG" --verbose > "$LOG_DIR/health-monitor.log" 2>&1 &
MONITOR_PID=$!
